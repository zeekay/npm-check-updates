...
// Returns true if the given dependency is a valid range that is not a wild card.
// This ensures that hardcoded dependencies such as github urls are ignored.
function isValid(dep) {
    // do not upgrade non-npm version declarations (such as git tags)
    // do not upgrade versionUtil.wildcards
    return semver.validRange(dep) && !versionUtil.isWildCard(dep);
}
